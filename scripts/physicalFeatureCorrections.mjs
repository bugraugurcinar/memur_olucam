export const physicalFeatureCoordinateCorrections = {
  plain_cukurova: {
    lat: 36.91,
    lon: 35.33,
    sourceDisplayName: "Çukurova, Seyhan-Ceyhan delta ovası, Adana/Mersin, Türkiye",
  },
  plain_elmali_ovasi: {
    lat: 36.735,
    lon: 29.917,
    sourceDisplayName: "Elmalı Ovası, Elmalı, Antalya, Türkiye",
  },
  plain_eskisehir_ovasi: {
    lat: 39.78,
    lon: 30.53,
    sourceDisplayName: "Eskişehir Ovası, Eskişehir, Türkiye",
  },
  plain_murted_akinci_ovasi: {
    lat: 40.08,
    lon: 32.58,
    sourceDisplayName: "Mürted Ovası, Kahramankazan, Ankara, Türkiye",
  },
  plateau_catalca_kocaeli_platosu: {
    lat: 41.03,
    lon: 29.23,
    sourceDisplayName: "Çatalca-Kocaeli Platosu, İstanbul/Kocaeli, Türkiye",
  },
  plateau_persembe_platosu: {
    lat: 40.69,
    lon: 37.43,
    sourceDisplayName: "Perşembe Platosu, Aybastı, Ordu, Türkiye",
  },
  plateau_yildiz_platosu: {
    lat: 41.55,
    lon: 27.6,
    sourceDisplayName: "Yıldız Platosu, Trakya, Kırklareli çevresi, Türkiye",
  },
  plateau_canik_platosu: {
    lat: 40.92,
    lon: 36.6,
    sourceDisplayName: "Canik Platosu, Orta Karadeniz, Samsun/Tokat çevresi, Türkiye",
  },
  plateau_teke_platosu: {
    lat: 36.92,
    lon: 29.92,
    sourceDisplayName: "Teke Platosu, Teke Yarımadası, Antalya/Burdur/Muğla, Türkiye",
  },
  plateau_taseli_platosu: {
    lat: 36.76,
    lon: 32.72,
    sourceDisplayName: "Taşeli Platosu, Ermenek-Mut-Gülnar-Anamur çevresi, Türkiye",
  },
  plateau_goller_yoresi_platosu: {
    lat: 37.55,
    lon: 30.45,
    sourceDisplayName: "Göller Yöresi Platosu, Isparta/Burdur çevresi, Türkiye",
  },
  plateau_anamur_platosu: {
    lat: 36.17,
    lon: 32.84,
    sourceDisplayName: "Anamur Platosu, Mersin, Türkiye",
  },
  plateau_erzurum_kars_platosu: {
    lat: 40.15,
    lon: 42.65,
    sourceDisplayName: "Erzurum-Kars Platosu, Sarıkamış/Kars çevresi, Türkiye",
  },
  plateau_agri_platosu: {
    lat: 39.72,
    lon: 43.05,
    sourceDisplayName: "Ağrı Platosu, Ağrı çevresi, Türkiye",
  },
  plateau_tendurek_cevresi_lav_platosu: {
    lat: 39.35,
    lon: 43.87,
    sourceDisplayName: "Tendürek çevresi lav platosu, Ağrı/Van, Türkiye",
  },
  river_coruh_nehri: {
    lat: 41.22,
    lon: 41.72,
    sourceDisplayName: "Çoruh Nehri, Artvin/Borçka çevresi, Türkiye",
  },
  river_yesilirmak: {
    lat: 41.33,
    lon: 36.78,
    sourceDisplayName: "Yeşilırmak, Çarşamba deltası, Samsun, Türkiye",
  },
  river_kizilirmak: {
    lat: 41.65,
    lon: 35.95,
    sourceDisplayName: "Kızılırmak, Bafra deltası, Samsun, Türkiye",
  },
  river_filyos_yenice: {
    lat: 41.56,
    lon: 32.08,
    sourceDisplayName: "Filyos Çayı, Filyos/Zonguldak çevresi, Türkiye",
  },
  river_bartin_cayi: {
    lat: 41.64,
    lon: 32.33,
    sourceDisplayName: "Bartın Çayı, Bartın kıyı kesimi, Türkiye",
  },
  river_sakarya_nehri: {
    lat: 41.12,
    lon: 30.68,
    sourceDisplayName: "Sakarya Nehri, Karasu/Sakarya kıyı kesimi, Türkiye",
  },
  river_susurluk_cayi: {
    lat: 40.23,
    lon: 28.35,
    sourceDisplayName: "Susurluk/Simav Çayı, Marmara'ya dökülen kesim, Türkiye",
  },
  river_gonen_cayi: {
    lat: 40.11,
    lon: 27.64,
    sourceDisplayName: "Gönen Çayı, Gönen/Balıkesir çevresi, Türkiye",
  },
  river_biga_cayi: {
    lat: 40.33,
    lon: 27.31,
    sourceDisplayName: "Biga Çayı, Karabiga/Çanakkale çevresi, Türkiye",
  },
  river_nilufer_cayi: {
    lat: 40.18,
    lon: 29.05,
    sourceDisplayName: "Nilüfer Çayı, Bursa Ovası çevresi, Türkiye",
  },
  river_meric_nehri: {
    lat: 40.72,
    lon: 26.17,
    sourceDisplayName: "Meriç Nehri, Enez/Edirne çevresi, Türkiye",
  },
  river_bakircay: {
    lat: 38.94,
    lon: 26.94,
    sourceDisplayName: "Bakırçay, Çandarlı/Dikili deltası, İzmir, Türkiye",
  },
  river_gediz_nehri: {
    lat: 38.57,
    lon: 26.82,
    sourceDisplayName: "Gediz Nehri, Gediz deltası, İzmir, Türkiye",
  },
  river_kucuk_menderes_nehri: {
    lat: 37.95,
    lon: 27.27,
    sourceDisplayName: "Küçük Menderes Nehri, Selçuk/Pamucak çevresi, İzmir, Türkiye",
  },
  river_buyuk_menderes_nehri: {
    lat: 37.58,
    lon: 27.26,
    sourceDisplayName: "Büyük Menderes Nehri, Balat/Söke deltası, Aydın, Türkiye",
  },
  river_dalaman_cayi: {
    lat: 36.73,
    lon: 28.78,
    sourceDisplayName: "Dalaman Çayı, Ortaca/Dalaman kıyı kesimi, Muğla, Türkiye",
  },
  river_esen_cayi: {
    lat: 36.27,
    lon: 29.3,
    sourceDisplayName: "Eşen Çayı, Patara/Eşen deltası, Muğla/Antalya, Türkiye",
  },
  river_aksu_cayi: {
    lat: 36.86,
    lon: 30.86,
    sourceDisplayName: "Aksu Çayı, Antalya kıyı kesimi, Türkiye",
  },
  river_koprucay: {
    lat: 36.84,
    lon: 31.17,
    sourceDisplayName: "Köprüçay, Serik/Boğazkent kıyı kesimi, Antalya, Türkiye",
  },
  river_manavgat_cayi: {
    lat: 36.74,
    lon: 31.45,
    sourceDisplayName: "Manavgat Çayı, Manavgat kıyı kesimi, Antalya, Türkiye",
  },
  river_goksu_nehri: {
    lat: 36.31,
    lon: 33.93,
    sourceDisplayName: "Göksu Nehri, Silifke deltası, Mersin, Türkiye",
  },
  river_seyhan_nehri: {
    lat: 36.72,
    lon: 35.27,
    sourceDisplayName: "Seyhan Nehri, Çukurova kıyı kesimi, Adana, Türkiye",
  },
  river_ceyhan_nehri: {
    lat: 36.58,
    lon: 35.74,
    sourceDisplayName: "Ceyhan Nehri, Yumurtalık/Adana kıyı kesimi, Türkiye",
  },
  river_asi_nehri: {
    lat: 36.1,
    lon: 35.94,
    sourceDisplayName: "Asi Nehri, Samandağ/Hatay kıyı kesimi, Türkiye",
  },
  river_firat_nehri: {
    lat: 37.03,
    lon: 37.98,
    sourceDisplayName: "Fırat Nehri, Birecik/Şanlıurfa çevresi, Türkiye",
  },
  river_dicle_nehri: {
    lat: 37.32,
    lon: 42.19,
    sourceDisplayName: "Dicle Nehri, Cizre/Şırnak çevresi, Türkiye",
  },
  river_murat_nehri: {
    lat: 38.76,
    lon: 41.5,
    sourceDisplayName: "Murat Nehri, Muş çevresi, Türkiye",
  },
  river_karasu_nehri: {
    lat: 39.95,
    lon: 41.17,
    sourceDisplayName: "Karasu Nehri, Erzurum çevresi, Türkiye",
  },
  river_buyuk_zap_suyu: {
    lat: 37.57,
    lon: 44.28,
    sourceDisplayName: "Büyük Zap Suyu, Hakkari çevresi, Türkiye",
  },
  river_aras_nehri: {
    lat: 39.87,
    lon: 44.33,
    sourceDisplayName: "Aras Nehri, Iğdır sınır kesimi, Türkiye",
  },
  river_kura_nehri: {
    lat: 41.11,
    lon: 42.7,
    sourceDisplayName: "Kura Nehri, Ardahan çevresi, Türkiye",
  },
  river_arpacay: {
    lat: 40.52,
    lon: 43.37,
    sourceDisplayName: "Arpaçay, Kars sınır kesimi, Türkiye",
  },
  coast_kuzey_ege_enine_kiyilari: {
    lat: 39.58,
    lon: 26.93,
    sourceDisplayName: "Kuzey Ege enine kıyıları, Edremit Körfezi çevresi, Türkiye",
  },
  coast_orta_ege_enine_kiyilari: {
    lat: 37.86,
    lon: 27.26,
    sourceDisplayName: "Orta Ege enine kıyıları, Kuşadası/Aydın çevresi, Türkiye",
  },
  coast_guney_ege_enine_kiyilari: {
    lat: 37.04,
    lon: 27.43,
    sourceDisplayName: "Güney Ege enine kıyıları, Bodrum/Muğla çevresi, Türkiye",
  },
  coast_datca_kiyilari: {
    lat: 36.73,
    lon: 27.68,
    sourceDisplayName: "Datça kıyıları, Muğla, Türkiye",
  },
  coast_kekova_cevresi: {
    lat: 36.2,
    lon: 29.86,
    sourceDisplayName: "Kekova çevresi, Demre/Antalya, Türkiye",
  },
  coast_kalkan_kas_kiyilari: {
    lat: 36.27,
    lon: 29.41,
    sourceDisplayName: "Kalkan-Kaş kıyıları, Antalya, Türkiye",
  },
  coast_terkos_kiyilari: {
    lat: 41.32,
    lon: 28.62,
    sourceDisplayName: "Terkos kıyıları, İstanbul, Türkiye",
  },
  coast_tasucu_cevresi: {
    lat: 36.32,
    lon: 33.89,
    sourceDisplayName: "Taşucu çevresi, Silifke/Mersin, Türkiye",
  },
  coast_narlikuyu_koylari: {
    lat: 36.45,
    lon: 34.11,
    sourceDisplayName: "Narlıkuyu koyları, Mersin, Türkiye",
  },
};
